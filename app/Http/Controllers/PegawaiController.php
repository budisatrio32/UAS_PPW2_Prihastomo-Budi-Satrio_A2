...
class PegawaiController extends Controller
{
    ...
    public function store(Request $request) {
        $validator = Validator::make($request->all(), [
            'pekerjaan_id' => 'required|exists:budi_534908_pekerjaan,id',
            'nama' => 'required|string|max:255',
            'email' => 'required|email|unique:budi_534908_pegawai,email',
    ...
    public function update(Request $request) {
        $validator = Validator::make($request->all(), [
            'id' => 'required|exists:budi_534908_pegawai,id',
            'pekerjaan_id' => 'required|exists:budi_534908_pekerjaan,id',
            'nama' => 'required|string|max:255',
            'email' => 'required|email|unique:budi_534908_pegawai,email,' . $request->id,
            'gender' => 'required|in:male,female',
            'is_active' => 'required|boolean',
        ]);
        ...
        if ($data->save()) {
            ...
            return redirect()->route('pegawai.index')->with('error', 'Data pegawai gagal diupdate');
        }
    }
